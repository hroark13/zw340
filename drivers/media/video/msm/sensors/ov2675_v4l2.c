 ...
 *
 */
/***********ZTEBSP yuxin create this file for sensor ov2675,2012.10.19++****************/
...
/*ZTEBSP yuxin copy from SH code 2012.12.18 ++*/
...
//ZTEBSP yuxin modify image quality 2012.12.06++
//Color Matrix1
{0x3320, 0xfa },//  
{0x3321, 0x11 },//  
{0x3322, 0x92 },//  
...
//@@ Color saturation s115 c13 e13 new
{0x3394, 0x42},//;45
{0x3395, 0x42 },//;45

//@@ Color saturation ON new
{0x3301, 0xFF },//
{0x3391, 0x06 },//
...
//    printk("Preview_Maxlines=%d\n", ov2675_preview_maxlines);
   ...
   /*ZTEBSP yuxin copy from SH code 2012.12.18 ++*/
    ret_h = ret_m = ret_l = 0;
     rc = msm_camera_i2c_read(s_ctrl->sensor_i2c_client,0x3028, 
	 	                                    &ret_h,MSM_CAMERA_I2C_BYTE_DATA);
     rc = msm_camera_i2c_read(s_ctrl->sensor_i2c_client,0x3029, 
		                                    &ret_l,MSM_CAMERA_I2C_BYTE_DATA);
     rc = msm_camera_i2c_read(s_ctrl->sensor_i2c_client,0x302c, 
	 	                                    &ret_m,MSM_CAMERA_I2C_BYTE_DATA);
    ...
    rc = msm_camera_i2c_read(s_ctrl->sensor_i2c_client,0x302b, 
		                                   &ret_l,MSM_CAMERA_I2C_BYTE_DATA);
    rc = msm_camera_i2c_read(s_ctrl->sensor_i2c_client,0x302d, 
		                                   &ret_m,MSM_CAMERA_I2C_BYTE_DATA);
...
static int ov2675_set_capture_exposure_gain(struct msm_sensor_ctrl_t *s_ctrl)
{
   ...
   // iCapture_Gain = ov2675_gain*(2400)/(3600);
        iCapture_Gain = ov2675_gain*(750)/(1000);  //yuxin modify for prev and snap brightness ot the same 2012.11.02-2
...
{
    printk("%s: enter\n",__func__);
    msm_camera_i2c_write(s_ctrl->sensor_i2c_client,0x3606,
              ...
              switch(res)
	       {
	          case 0://snapshot
				...
				rc = msm_camera_i2c_write(s_ctrl->sensor_i2c_client, 0x3301, ((temp_2)&0xbf), 
					                                        MSM_CAMERA_I2C_BYTE_DATA);
				//printk("%s: ****yuxin add read 0x330f out,temp_1 is 0x%x\n",__func__,temp_1);
				temp_1 = temp_1 & 0x1f;
				temp_1 = temp_1 * 2;
				//printk("%s: ####yuxin add read 0x330f out,temp_1 is 0x%x\n",__func__,temp_1);
			      ...
			      */
  			     ...
  			     // rc = msm_camera_i2c_read(s_ctrl->sensor_i2c_client,0x339a, &bns,    
  			      ...
  			      rc = msm_camera_i2c_read(s_ctrl->sensor_i2c_client,0x301b, &avrg,   
				  	                                        MSM_CAMERA_I2C_BYTE_DATA);
                         //ZTEBSP yuxin del for low light environment,the more pictures taken,the brighter ++
				#if 0
		   ...
		   default :
		   	{
				...
				//ZTEBSP yuxin turn on AEC/AGC when preview 2012.11.28
				 ...
				 v4l2_subdev_notify(&s_ctrl->sensor_v4l2_subdev,
		    			NOTIFY_PCLK_CHANGE,
		    			&s_ctrl->sensordata->pdata->ioclk.vfe_clk_rate);
				 //ZTEBSP yuxin add this delay for avoid preview flicker because 
				 //ov2675 AEC speed is too slow.
				 msleep(500);
		            	s_ctrl->func_tbl->sensor_start_stream(s_ctrl);
...
/***************ZTEBSP yuxin add for ov2675 setting effects,2012.11.02 ++************/
...
static struct msm_sensor_fn_t ov2675_func_tbl = {
	.sensor_start_stream =ov2675_sensor_start_stream ,//msm_sensor_start_stream,
	.sensor_stop_stream =ov2675_sensor_stop_stream ,//msm_sensor_stop_stream,
