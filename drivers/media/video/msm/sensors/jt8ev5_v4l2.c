...
// zte-modify, 20120904 fuyipeng mdoify for awb +++
...
//ZTEBSP DangXiao 20120525, for T8EV5, AF firmware download,start
static struct msm_camera_i2c_conf_array jt8ev5_fw_settings[] = {


};
//ZTEBSP DangXiao 20120525, for T8EV5, AF firmware download,end
...
//ZTEBSP DangXiao 20120523, T8EV5 , start
...
//ZTEBSP DangXiao 20120523, T8EV5 , end
...
//ZTEBSP DangXiao 20120525, start
...
struct msm_sensor_v4l2_ctrl_info_t jt8ev5_v4l2_ctrl_info[] = {
...

	{
        ...
        .s_v4l2_ctrl = jt8ev5_awb_sensor_s_ctrl_by_enum, // zte-modify, 20120904 fuyipeng mdoify for awb
	},	
...
static int jt8ev5_pwdn_gpio;
static int jt8ev5_reset_gpio;
#if (defined CONFIG_PROJECT_P825F01) || (defined CONFIG_PROJECT_P865F01) || (defined CONFIG_PROJECT_P865V30) ||defined ( CONFIG_PROJECT_P865V20)
static int jt8ev5_power_moto_gpio;//zhangzhao for mecnex sensor power moto
#endif
static void  jt8ev5_release_gpio(void)
{
	pr_err("%s release gpio %d, %d\n", __func__,jt8ev5_pwdn_gpio,jt8ev5_reset_gpio);
	gpio_free(jt8ev5_pwdn_gpio);
	gpio_free(jt8ev5_reset_gpio);
#if (defined CONFIG_PROJECT_P825F01) || (defined CONFIG_PROJECT_P865F01) || (defined CONFIG_PROJECT_P865V30) ||defined ( CONFIG_PROJECT_P865V20)
	gpio_free(jt8ev5_power_moto_gpio);
#endif
}

static void jt8ev5_hw_reset(void)
{
	CDBG("--CAMERA-- %s ... (Start...)\n", __func__);
	gpio_set_value(jt8ev5_reset_gpio, 1);   /*reset camera reset pin*/
	//usleep_range(5000, 5100);
	usleep_range(5000, 5100);
	gpio_set_value(jt8ev5_reset_gpio, 0);
	usleep_range(8000, 8100);
	gpio_set_value(jt8ev5_reset_gpio, 1);
	usleep_range(5000, 5100);
	CDBG("--CAMERA-- %s ... (End...)\n", __func__);
}

static int jt8ev5_probe_init_gpio(const struct msm_camera_sensor_info *data)
{
	int rc = 0;
       CDBG("%s: ++++++++++++++++entered\n", __func__);

	jt8ev5_pwdn_gpio = data->sensor_pwd;
	jt8ev5_reset_gpio = data->sensor_reset ;
#if (defined CONFIG_PROJECT_P825F01) || (defined CONFIG_PROJECT_P865F01) || (defined CONFIG_PROJECT_P865V30) ||defined ( CONFIG_PROJECT_P865V20)
	jt8ev5_power_moto_gpio = data->power_moto_gpio;
#endif	
	CDBG("%s: pwdn_gpio:%d, reset_gpio:%d\n", __func__,
			jt8ev5_pwdn_gpio, jt8ev5_reset_gpio);

	rc = gpio_request(jt8ev5_pwdn_gpio, "jt8ev5-pwd");
	if (rc < 0)
		{
		pr_err("%s: gpio_request jt8ev5_pwdn_gpio failed!",
			 __func__);
		goto gpio_request_failed;
		}

	CDBG("gpio_tlmm_config %d\r\n", jt8ev5_pwdn_gpio);
	rc = gpio_tlmm_config(GPIO_CFG(jt8ev5_pwdn_gpio, 0,
	...
	gpio_direction_output(jt8ev5_pwdn_gpio, 1);

	rc = gpio_request(jt8ev5_reset_gpio, "jt8ev5-reset");
	if (rc < 0)
		{
		pr_err("%s: gpio_request jt8ev5_reset_gpio failed!",
			 __func__);
		goto gpio_request_failed2;
		}

	CDBG("gpio_tlmm_config %d\r\n", jt8ev5_reset_gpio);
	rc = gpio_tlmm_config(GPIO_CFG(jt8ev5_reset_gpio, 0,
		GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN,
		GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	if (rc < 0) {
	...
	gpio_direction_output(jt8ev5_reset_gpio, 1);
...
static void jt8ev5_power_on(void)
{
	pr_err("%s :+++++++++++++++++=\n", __func__);
#if (defined CONFIG_PROJECT_P825F01) || (defined CONFIG_PROJECT_P865F01) || (defined CONFIG_PROJECT_P865V30) ||defined ( CONFIG_PROJECT_P865V20)
	gpio_set_value(jt8ev5_power_moto_gpio, 1);   	//ZTEBSP DangXiao for T8EV5
#endif	
	...
	usleep_range(5000, 5100);
	gpio_set_value(jt8ev5_pwdn_gpio, 1);   	//ZTEBSP DangXiao for T8EV5
	//gpio_set_value(jt8ev5_reset_gpio, 1);
	usleep_range(5000, 5100);
	CDBG("%s: ---------------------%d\n", __func__, __LINE__);
}
...
static void jt8ev5_power_down(void)
{
	pr_err("%s\n", __func__);
	gpio_set_value(jt8ev5_pwdn_gpio, 0);	//ZTEBSP DangXiao for T8EV5
	//gpio_set_value(jt8ev5_reset_gpio, 0);
	usleep_range(5000, 5100);
	//gpio_set_value(39, 1);//zhangzhao
	...
	usleep_range(5000, 5100);
#if (defined CONFIG_PROJECT_P825F01) || (defined CONFIG_PROJECT_P865F01) || (defined CONFIG_PROJECT_P865V30) ||defined ( CONFIG_PROJECT_P865V20)
	gpio_set_value(jt8ev5_power_moto_gpio, 0);   	//ZTEBSP DangXiao for T8EV5
#endif	
}

int32_t jt8ev5_sensor_power_up(struct msm_sensor_ctrl_t *s_ctrl)
{
	int32_t rc = 0;
...
// zte-modify, 20120831 fuyipeng modify for AF Rect for touch focus ---
int32_t jt8ev5_sensor_power_down(struct msm_sensor_ctrl_t *s_ctrl)
{
	int32_t rc = 0;
			...
			int update_type, int res)
{
	...
	/*[ECID:000000] ZTEBSP wangbing, for camera soc flash, 20120702 */
			...
			default :
                ...
                // zte-modify, 20120904 fuyipeng mdoify for awb should set before scene +++
...
static struct msm_sensor_fn_t jt8ev5_func_tbl = {
	...
	.sensor_power_up = jt8ev5_sensor_power_up, //msm_sensor_power_up,
	.sensor_power_down = jt8ev5_sensor_power_down,//msm_sensor_power_down,
    .sensor_set_af_rect = jt8ev5_sensor_set_af_rect,// zte-modify, 20120831 fuyipeng modify for AF Rect
